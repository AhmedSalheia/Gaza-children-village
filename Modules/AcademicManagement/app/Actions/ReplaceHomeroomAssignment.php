...
/**
 * Atomically supersede an existing active homeroom assignment and create a
 * replacement for the same class group.
 *
 ...
 *     Closed/archived semesters deny all mutations.
 *  3. The replacedOn date must be on or after the old assignment's starts_on.
 */
final class ReplaceHomeroomAssignment
{
    private const MUTABLE_STATUSES = ['open', 'draft'];

    public function __construct(
        private readonly CreateHomeroomAssignment $createAction,
    ) {}

    public function __invoke(
        HomeroomAssignment $old,
        ...
        return DB::transaction(function () use ($old, $newStaffPositionId, $replacedOn, $reason, $isCoLead): HomeroomAssignment {
            $locked = HomeroomAssignment::lockForUpdate()->findOrFail($old->id);
            ...
            if ($lockedStatus !== AssignmentStatus::Active) {
                throw new AssignmentException(
                    "HomeroomAssignment #{$old->id} has status '{$lockedStatus->value}' and is ".
            ...
            return ($this->createAction)(
                ...
                startsOn: $replacedOn,
                isCoLead: $isCoLead ?? (bool) $locked->is_co_lead,
            );
        });
    }
