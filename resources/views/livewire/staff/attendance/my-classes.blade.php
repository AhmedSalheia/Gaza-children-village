<div class="p-6 space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-800">My Classes — Daily Attendance</h1>
    ...
    @if($classes->isEmpty())
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-5 text-sm text-yellow-800">
            No homeroom assignments found for your account in the current semester.
            Contact the administration if you believe this is incorrect.
        </div>
    @else
                ...
                <div class="bg-white rounded-lg shadow p-5 space-y-3">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">{{ $class->level_name }}
                            @if($class->is_co_lead)
                                · <span class="text-yellow-600">Co-lead</span>
                            @else
                                · <span class="text-blue-600">Lead</span>
                            @endif
                        </p>
                    </div>
                    ...
                    @if($class->sheet_id)
                        <a href="{{ route('staff.attendance.sheet', ['sheetId' => $class->sheet_id]) }}"
                           class="block w-full text-center px-4 py-2 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700">
                            {{ in_array($class->sheet_status, ['draft', 'returned']) ? 'Continue Entry' : 'View Sheet' }}
                        </a>
                    @else
