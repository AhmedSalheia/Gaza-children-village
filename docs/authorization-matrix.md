...
3. **No inferred teaching access**: A `teacher` position grants the `teacher` role, which gives `institution_semester.view`, `operational_period.view`, and `person.view` only. No class, student, or mark access is implied by a position alone.
